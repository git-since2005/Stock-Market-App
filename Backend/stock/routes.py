...
@router.post("/predict-tomorrow")
def endpoint_predict(request: dict):
    ticker = request.get("ticker")
    if not ticker:
        raise HTTPException(status_code=400, detail="Ticker is required")
    price = predict_tomorrow_price(ticker)
    return {"ticker": ticker.upper(), "predicted_price": round(price, 2)}
